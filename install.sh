...
echo ""
if [[ -n "${CCM_META:-}" ]]; then
  echo "Bereit! Navigiere in dein Projekt und führe aus:"
  echo "  ccm add"
  echo ""
  echo "Das registriert das Projekt im Meta-Repo und erstellt eine CLAUDE.md."
else
  echo "Nächste Schritte:"
  echo ""
  echo "  1. Meta-Repo einrichten (einmalig):"
  echo "     mkdir mein-projekt-meta && cd mein-projekt-meta"
  echo "     ccm init"
  echo ""
  echo "  2. Projekt registrieren (pro Repo):"
  echo "     cd ~/mein-projekt"
  echo "     ccm add"
  echo ""
  echo "  Mehr: https://github.com/$CCM_REPO"
fi
